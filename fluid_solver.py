...
def init(rho: wp.array2d(dtype=float), u: wp.array2d(dtype=wp.vec2), radius: int, dir: wp.vec2):
    i, j = wp.tid()
    # i=row (vertical index), j=col (horizontal index)

    # Now, want to create a new circle, the obstacle
    obstacle_x = grid_height/3
    obstacle_y = grid_width/2
    dist_from_obstacle = wp.length(wp.vec2(float(i - obstacle_y), float(j - obstacle_x)))

    if dist_from_obstacle < (2*radius):
        rho[i, j] = 1.0
        u[i, j] = wp.vec2(0.0, 0.0)

    else:
        center_x = 0  # Move the circle's center to the leftmost (x = radius)
        center_y = grid_width/2  # Keep the y-center at the middle of the grid

        #d = wp.length(wp.vec2(float(i - grid_width / 2), float(j - grid_height / 2)))
        # Calculate the distance from each grid point (i, j) to the new center
        d = wp.length(wp.vec2(float(i - center_y), float(j - center_x)))
...
@wp.kernel
# init without obstacle
def init_(rho: wp.array2d(dtype=float), u: wp.array2d(dtype=wp.vec2), radius: int, dir: wp.vec2, obstacle_map: wp.array2d(dtype=int)):
    i, j = wp.tid()
    # i=row (vertical index), j=col (horizontal index)
